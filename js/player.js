...
$.Player = function(playerId, color) {
	...
	this.vx = 0;
	this.vy = 0;
	this.vmax = $.definitions.players.vmax;
	this.direction = 0;
	this.accel = 0.5;
	this.radius = 10;
	this.life = 1;
...
$.Player.prototype.update = function() {
	if( this.life > 0 ) {
		...
		Apply Forces
		==============================================================================*/
		if( this.control.state.up ) {
			this.vy -= this.accel * $.dt;
			if( this.vy < -this.vmax ) {
				this.vy = -this.vmax;
			}
		} else if( this.control.state.down ) {
			this.vy += this.accel * $.dt;
			if( this.vy > this.vmax ) {
				this.vy = this.vmax;
			}
		...
		} else if( this.control.state.right ) {
			this.vx += this.accel * $.dt;
			if( this.vx > this.vmax ) {
				this.vx = this.vmax;
			}
		...
		if( this.weapon.fireRateTick < this.weapon.fireRate ){
			...
			if( $.autofire || ( !$.autofire && this.control.state.shoot ) ){
				$.audio.play( 'shoot' );
